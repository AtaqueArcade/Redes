...
public class NanoGameServer implements Runnable {
	...
	public void run() {
		try {
			while (true) {
				...
				// accept returns a socket to exchange messages with the
				// incoming client
				Socket s = serverSocket.accept();
				System.out.println("New client connected from " + s.getInetAddress().toString() + ":" + s.getPort());

				// A new thread is started and it receives the shared status,
				// the socket and the broker address
	...
	public void init() {
		try {
			// We create the server socket and it is bound to the address and
			// port specified by socketAddress
