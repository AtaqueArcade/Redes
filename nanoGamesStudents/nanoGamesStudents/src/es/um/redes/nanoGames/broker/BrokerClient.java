 ...
 * espec�ficas. En la actual versi�n s�lo soporta una funci�n de
 * consulta sobre el UPTIME del host.
 */
public class BrokerClient {
	 ...
	 */
	public BrokerClient(String agentAddress) throws SocketException {
		// Registrar dirección del servidor
		this.agentAddress = new InetSocketAddress(agentAddress, DEFAULT_PORT);
	...
	public long getToken() throws IOException {
		...
		// Enviar solicitud
		DatagramPacket packetsolicitud = new DatagramPacket(req, req.length, agentAddress);
		socket.send(packetsolicitud);
		// Recibir respuesta
		byte[] response = new byte[PACKET_MAX_SIZE];
		DatagramPacket packetrespuesta = new DatagramPacket(response, response.length);
		socket.setSoTimeout(1000);
		socket.receive(packetrespuesta);
		// Extraer TimeTicks (Token)
		long token = getTimeTicks(packetrespuesta.getData());
		// Devolver token
		socket.close();
		return token;
