...
public class NGCommands {
	...
	/**
	 * The list of valid commands that can be entered by the user of this shell,
	 * with their corresponding string and help message. Note the matching
	 * order!
	 */
	private static final Byte[] _valid_user_commands = { COM_ROOMLIST, COM_ENTER, COM_NICK, COM_ANSWER, COM_EXIT,
			COM_RULES, COM_STATUS, COM_QUIT, COM_HELP };

	/**
	 * Accepted string for each command
	 */
	private static final String[] _valid_user_commands_str = { "roomlist", "enter", "nick", "a", "exit", "rules",
			"status", "quit", "help" };

	/**
	 * Help message for each command
	 */
	private static final String[] _valid_user_commands_help = { "provides a list of available rooms to play",
			"enter a particular <room>", "to set the <nickname> in the game", "<answer> to the challenge",
			"to leave the current room", "gives information about the rules of the game",
			"shows the status of the game", "to quit the application", "shows this information" };

	/**
	 * Translates a string to its corresponding command.
	 * 
	 * @param str
	 *            The string entered by the user of this shell.
	 * @return The corresponding command code (COM_xxx), or COM_INVALID if not a
	 *         valid keyword any valid user command in _valid_user_commands_str.
	 */
	...
	/**
	 * Prints the list of valid commands and brief help message for each
	 * command.
	 */
