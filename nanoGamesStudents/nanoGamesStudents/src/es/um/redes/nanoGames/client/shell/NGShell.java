...
public class NGShell {
	...
	private String[] readGeneralCommandFromStdIn() {
		...
		while (true) {
			...
			// we translate the string provided by the user to a byte
			...
			case NGCommands.COM_ROOMLIST:
				// These commands are valid and they do not have parameters (if
	...
	// Uses the standard input to obtain the command typed by the user and
	// checks if there is data available in the socket
	private String[] readGameCommandFromStdIn(NGGameClient ngclient) {
		...
		while (true) {
			...
			String input = "";
			// this loop runs until the user provides a command or the socket
			// has incoming data
			while (blocked) {
				try {
					if (ngclient.isDataAvailable()) {
						// if the socket has data then we set the current
						// command as SOCKET_IN and return
						System.out.println("* Message received from server...");
						command = NGCommands.COM_SOCKET_IN;
						return null;
					} else if (standardInput.ready()) {
						// if the user type one line the we read the line and
						// exit the loop
			...
			// In case the user typed a command, we check it as we did
			// previously in readGeneralCommandFromStdIn()
			StringTokenizer st = new StringTokenizer(input);
			if (st.hasMoreTokens() == false) {
				continue;
			}
	...
	private boolean validateCommandArguments(String[] args) {
		...
		case NGCommands.COM_ANSWER:
			if (args.length == 0) {
				System.out.println("Correct use: a <answer>");
				return false;
			}
			break;
		default:
